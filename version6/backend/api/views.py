...
def upload_images(request):
    ...
    clustered_data = {i: [] for i in range(n_clusters)}
    for idx, cluster in enumerate(clusters):
        clustered_data[cluster].append(image_paths[idx])

    # Prepare data for visualization
    nodes = [{"id": f"Image-{i}", "file_path": image_paths[i]} for i in range(len(image_paths))]
    edges = []
    for cluster_id, paths in clustered_data.items():
        cluster_nodes = [i for i, path in enumerate(image_paths) if path in paths]
        for i in range(len(cluster_nodes)):
            for j in range(i + 1, len(cluster_nodes)):
                edges.append({"source": f"Image-{cluster_nodes[i]}", "target": f"Image-{cluster_nodes[j]}"})
